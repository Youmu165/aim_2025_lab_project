...
import com.aim.metaheuristics.population.geneticoperators.hillclimbing.DBHC_IE;
import com.aim.metaheuristics.population.geneticoperators.mutation.BitMutation;
import com.aim.metaheuristics.population.geneticoperators.NoopHeuristic;
...
public class Lab5ExercisesRunner extends TestFrame {

	private final String BOXPLOT_TITLE;

	public Lab5ExercisesRunner(TestFrameConfig config) {

		super(config);

		Lab5ExercisesTestFrameConfig oTestConfiguration = (Lab5ExercisesTestFrameConfig) getTestConfiguration();
	...
	public List<RunData> runExperiment(int heuristicId, int trialId) {
		...
		Lab5ExercisesTestFrameConfig config = (Lab5ExercisesTestFrameConfig) getTestConfiguration();
		...
		CrossoverHeuristic crossover = null;
		switch(config.CROSSOVER) {
			case ONE_PTX -> crossover = new OnePTX(sat, random);
			case UXO -> crossover = new UniformXO(sat, random);
		}

		PopulationHeuristic localSearch = null;
		switch (config.MODE) {
			case GA -> localSearch = new NoopHeuristic(sat, random);
			// TODO see here - sets the hill climbing operator for the memetic algorithm
			case MA -> localSearch = new DBHC_IE(sat, random);
		}

		PopulationReplacement replacement = null;
		switch (config.REPLACEMENT) {
            case BASIC -> replacement = new BasicReplacement();
            case TRANS_GENERATIONAL -> replacement = new TransGenerationalReplacementWithElitistReplacement();
        }

		ParentSelection p1Selection = null, p2Selection = null;
		switch(config.P1_SELECTION) {
            case TOURNAMENT -> p1Selection = new TournamentSelection(sat, random, POP_SIZE, config.TOURNAMENT_SIZE);
            case RANDOM -> p1Selection = new RandomSelection(sat, random, POP_SIZE);
            case FITTEST -> p1Selection = new FittestSelection(sat, random, POP_SIZE);
        }

		switch(config.P2_SELECTION) {
			case TOURNAMENT -> p2Selection = new TournamentSelection(sat, random, POP_SIZE, config.TOURNAMENT_SIZE);
			case RANDOM -> p2Selection = new RandomSelection(sat, random, POP_SIZE);
			case FITTEST -> p2Selection = new FittestSelection(sat, random, POP_SIZE);
		}
	...
	public void runTests() {
		...
		// create and show the boxplot
		String strSearchMethod = ((Lab5ExercisesTestFrameConfig)getTestConfiguration()).MODE.name();
		XBoxPlot.getPlotCreator().createPlot(BOXPLOT_TITLE, strSearchMethod, "Objective Value", oPlotData);
	...
	public static void main(String[] args) {
		...
		TestFrameConfig config = Lab5ExercisesTestFrameConfig.getInstance();
		TestFrame runner = new Lab5ExercisesRunner(config);
		runner.runTests();
	}
}
