 ...
 * 
 * Note - This will take longer than previous algorithms to run since
 ...
 *
 */
public class Lab4ExercisesRunner extends TestFrame {
	 ...
	 * @param config
	 */
	public Lab4ExercisesRunner(TestFrameConfig config) {
	...
	public void runTests() {
		...
		List<Double> list = oRunData.stream().mapToDouble(RunData::getBestSolutionValue).sorted().boxed().toList();
		int len = list.size();
		double median;
		if(len % 2 == 0) {
			median = (list.get(len/2) + list.get((len/2)+1)) / 2;
		} else {
			median = list.get(len/2);
		}
		System.out.println("Median of whole sample: " + median);
		...
		for (int id : oHeuristicIds) {
			...
			XYChart plot = XLineChart.getPlotCreator().createChart(strTitle, strXLabel, strYLabel, oProgressPlotData);
			plot.getSeriesMap().forEach( (k,v) -> System.out.println(k));
			plot.getSeriesMap().get(strBest).setLineColor(Color.BLUE);
			plot.getSeriesMap().get(strWorst).setLineColor(Color.RED);
		}
	}

	String strBest = null;
	String strWorst = null;
	...
	private void addBestTrace(List<RunData> oRunData, List<PlotData> oProgressPlotData, int id) {
		...
		strBest = String.format("Best Trial #%d", data.getTrialId());
		oProgressPlotData.add(new PlotData(data.getData(), strBest));
	}
	...
	private void addWorstTrace(List<RunData> oRunData, List<PlotData> oProgressPlotData, int id) {
		...
		RunData data = oRunData.stream().filter(f -> f.getHeuristicId() == id).filter(f -> f.getBestSolutionValue() == best).findFirst().get();
		strWorst = String.format("Worst Trial #%d", data.getTrialId());
		oProgressPlotData.add(new PlotData(data.getData(), strWorst));
	}
	...
	public List<RunData> runExperimentsForHeuristicId(int heuristicId) {
		...
		Stream<RunData> dat = runUsingExperimentalParallelism(rangeAsStream(0, getTestConfiguration().getTotalRuns() - 1)).map( run -> {
			...
			SearchMethod searchMethod = new SimulatedAnnealing(((Lab4ExercisesTestFrameConfig)getTestConfiguration())
	...
	public static void main(String [] args) {
		...
		Lab4ExercisesTestFrameConfig config = Lab4ExercisesTestFrameConfig.getInstance();
		TestFrame runner = new Lab4ExercisesRunner(config);
		runner.runTests();
	}
}
