...
//		problem.getMeme(solutionIndex, 1).setMemeOption(rng.nextInt(4));
		for (int memeIndex = 0; memeIndex < problem.getNumberOfMemes(); memeIndex++) {

			if (rng.nextDouble() < innovationRate) {
				Meme meme = problem.getMeme(solutionIndex, memeIndex);
				int currentOption = meme.getMemeOption();
				int totalOptions = meme.getTotalOptions();


				int newOption;
				do {
					newOption = rng.nextInt(totalOptions);
